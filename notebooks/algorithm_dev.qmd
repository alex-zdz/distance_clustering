...
c_salso_vi <- salso(eam, loss = VI())

c_salso_binder <- salso(eam, loss = binder())

plot(y, col = cluster_true)
plot(y, col = c_salso_binder)
...
#### Test the initialization result:

```{r}

posterior_params <- prepare_AM_posterior_params(posterior_draws)
...
# Continue later: if prepare params function is in place, unify naming for uv and mv case to avoid later distinction:
# therefore sigma is always called sig2_post, even in the mv case, or better var_post?
...
c_salso_binder <- salso(clustering_matrix, loss = binder(), maxZealousAttempts = 0)

par(mfrow=c(2, 1))
plot(y, col = c_salso_binder)
plot(y, col = c_current)
...
results <- run_clustering(data = y,
               clustering_matrix = clustering_matrix,
               posterior_params = mix_post_draws,   
               prior_list,
               method         = "Wasserstein",
               n_runs         = 1,
               n_sweet = 1,
