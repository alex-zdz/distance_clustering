mixture_posterior <- function(c_alloc, data, prior_list){
  ...
  for (k in 1:K) {
    ...
    #sigma2_post[k] = b_post/(a_post -1) # this just sets sigma^2 to its expected value
    sigma2_post[k] = invgamma::rinvgamma(n = 1, shape = a_post, rate = b_post)
  }
  #weights= alpha_post/sum(alpha_post), again just the expected value!
  weights = rdirichlet(n=1, alpha_post) 
