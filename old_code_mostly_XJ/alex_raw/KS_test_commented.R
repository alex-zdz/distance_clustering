...
KS_distance_mixture <- function(observed_data, w, u, s, cdf_func){
  ...
  ecdf_func <- function(data) { 
    ...
    sorted <- sort(data) 
    ecdf <- rep(0, Length) 
    for (i in 1:Length) { # before 1:n
      ecdf[i] <- sum(sorted <= data[i]) / Length 
    } 
    return(ecdf) 
  } 
  
  # Error: n already used in ecdf_func, Length and n the same
  n = length(observed_data)
  FN = ecdf_func(observed_data)
  FN_sort = sort(FN)
  knots_vec = sort(observed_data)
...
#mixture_posterior(c_alloc_test, y, prior_list)
...
alpha = 1; kappa0=0.1; theta = 1;a = 1; b=1
