# Re-writing the salso algorithm using the minimum distance estimators
...
Binder_loss_posterior_expectation <- function(c_estimate, c_sample, pi_mat, a=1, b=1){
  # c_estimate: n vector
  # c_sample: I_mcmc * n matrix
  ...
  for (i in 1:(n-1)) {
    for(j in (i+1):n){
      ...
      # print(j)
      # print(dim(pi_mat))
      I_ij = (c_estimate[i]==c_estimate[j])*1.0
      f = f + I_ij*(pi_mat[i,j] - b/(a+b))
    }
  }
  return(-f)
}
