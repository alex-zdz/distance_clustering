...
# Mixture variance
mixture_variance <- function(param_post) {
  w <- param_post$weights
  mu <- param_post$mu_post
  sig2 <- param_post$sigma2_post
  
  # Within-component variance
  var_within <- sum(w * sig2)
  
  # Between-component variance
  mean_mixture <- sum(w * mu)
  var_between <- sum(w * (mu - mean_mixture)^2)
